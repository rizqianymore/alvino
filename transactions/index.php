...
// Check permission for modifications (Staff and Warehouse Manager can add transactions)
            ...
            if (empty($error)) {
                if ($item_id <= 0 || $qty <= 0 || !in_array($tipe, ['Masuk', 'Keluar'])) {
                    ...
                    try {
                        ...
                        mysqli_stmt_bind_param($ins_stmt, "iiiisss", $user_id, $item_id, $supplier_id, $tanggal, $qty, $tipe, $keterangan);
                        
                        if (!mysqli_stmt_execute($ins_stmt)) {
                            mysqli_stmt_close($ins_stmt);
                            throw new Exception("Gagal menyimpan riwayat transaksi: " . mysqli_error($conn));
                        }
                        mysqli_stmt_close($ins_stmt);
...
if (!empty($search)) {
    ...
    $query = "SELECT transactions.*, items.nama_barang, items.kode_barang, users.name as operator, suppliers.supplier_name 
              ...
              WHERE items.nama_barang LIKE ? OR items.kode_barang LIKE ? OR transactions.tipe LIKE ? OR suppliers.supplier_name LIKE ?
...
// Fetch all suppliers for form dropdown
...
<head>
    ...
    <script>
        // Simple client side helper to show/hide supplier based on transaction type
        function toggleSupplierField() {
            var tipe = document.getElementById('tipe').value;
            var supplierGroup = document.getElementById('supplier-form-group');
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Form Entry Transaksi (Only Staff & Manager) -->
        <?php if ($can_edit): ?>
            <div class="form-modal-inline">
                ...
                <form action="/transactions/index.php" method="POST" style="margin-top: 15px;">
                    ...
                    <div id="supplier-form-group" class="form-group" style="display: none;">
                        <label for="supplier_id">Pilih Supplier (Wajib untuk Barang Masuk)</label>
                        <select id="supplier_id" name="supplier_id" class="form-control">
                            <option value="">-- Pilih Supplier --</option>
                            <?php foreach ($suppliers as $sup): ?>
                                <option value="<?= esc($sup['id']) ?>"><?= esc($sup['supplier_name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
        ...
        <div class="actions-row">
            ...
            <form action="/transactions/index.php" method="GET" class="search-form">
                <input type="text" name="search" class="form-control" style="width: auto;" placeholder="Cari Barang, Tipe, Supplier..." value="<?= esc($search) ?>">
