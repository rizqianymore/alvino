...
// Check permission for modifications (Staff and Warehouse Manager can add/edit, only Warehouse Manager can delete)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $can_edit) {
    ...
    if (empty($error)) {
        ...
        if (isset($_POST['add_supplier'])) {
            ...
            if (empty($supplier_name) || empty($address) || empty($phone_number) || empty($email)) {
                $error = "Semua kolom input wajib diisi.";
            } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $error = "Format email tidak valid.";
            } else {
                $stmt = mysqli_prepare($conn, "INSERT INTO suppliers (supplier_name, address, phone_number, email) VALUES (?, ?, ?, ?)");
                mysqli_stmt_bind_param($stmt, "ssss", $supplier_name, $address, $phone_number, $email);
        ...
        if (isset($_POST['edit_supplier'])) {
            ...
            if (empty($supplier_name) || empty($address) || empty($phone_number) || empty($email)) {
                ...
                $stmt = mysqli_prepare($conn, "UPDATE suppliers SET supplier_name = ?, address = ?, phone_number = ?, email = ? WHERE id = ?");
                mysqli_stmt_bind_param($stmt, "ssssi", $supplier_name, $address, $phone_number, $email, $id);
                if (mysqli_stmt_execute($stmt)) {
        ...
        if (isset($_POST['delete_supplier']) && $can_delete) {
            ...
            $check_stmt = mysqli_prepare($conn, "SELECT id FROM transactions WHERE supplier_id = ? LIMIT 1");
            mysqli_stmt_bind_param($check_stmt, "i", $id);
            mysqli_stmt_execute($check_stmt);
            mysqli_stmt_store_result($check_stmt);
            
            if (mysqli_stmt_num_rows($check_stmt) > 0) {
                $error = "Gagal menghapus supplier. Data supplier masih digunakan dalam transaksi barang masuk.";
            } else {
...
if (!empty($search)) {
    $search_param = "%" . $search . "%";
    $query = "SELECT * FROM suppliers WHERE supplier_name LIKE ? OR address LIKE ? OR phone_number LIKE ? OR email LIKE ? ORDER BY supplier_name ASC";
                ...
                <form action="/suppliers/index.php" method="POST" style="margin-top: 15px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div class="form-group">
                            ...
                            <input type="text" id="phone_number" name="phone_number" class="form-control" placeholder="08xxxxxxxxxx" required>
                        </div>
                        <div class="form-group">
        ...
        ?>
            <div class="form-modal-inline">
                ...
                <form action="/suppliers/index.php" method="POST" style="margin-top: 15px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div class="form-group">
                            <label for="supplier_name_edit">Nama Supplier</label>
                            <input type="text" id="supplier_name_edit" name="supplier_name" class="form-control" value="<?= esc($edit_row['supplier_name']) ?>" required>
                        </div>
                        <div class="form-group">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                        <div class="form-group">
                            <label for="phone_number_edit">No. Telepon</label>
                            <input type="text" id="phone_number_edit" name="phone_number" class="form-control" value="<?= esc($edit_row['phone_number']) ?>" required>
                        </div>
                        <div class="form-group">
            ...
            <table>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            <td><?= $no++ ?></td>
                            <td><strong><?= esc($row['supplier_name']) ?></strong></td>
